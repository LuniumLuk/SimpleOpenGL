...
#include "SimpleGL/Core/Log.h"

namespace SGL {

    using Filepath = std::filesystem::path;

    inline Filepath assetRoot = "./Asset";
    inline Filepath shaderRoot = "./Shader";
    inline Filepath projRoot = ".";

    inline auto getAssetPath(Filepath const& path) noexcept -> Filepath {
        return assetRoot / path;
    }

    inline auto getShaderPath(Filepath const& path) noexcept -> Filepath {
        return shaderRoot / path;
    }

    inline auto getProjPath(Filepath const& path) noexcept -> Filepath {
        return projRoot / path;
    }

    inline auto readFile(Filepath const& path) noexcept -> std::string {
        ...
        if (ifs.is_open()) {
            ...
            SGL_LOG_ERROR("Failed to read file: {0}", path.string());
            return std::string();
        }
    }

    inline auto writeFile(Filepath const& path, void* data, size_t size) noexcept -> bool {
        ...
        if (ofs.is_open()) {
            ...
            SGL_LOG_ERROR("Failed to write file: {0}", path.string());
        }
        return false;
    }
